...
<body class="sb-nav-fixed">
   ...
   <h2 align="center" style="margin-right:50px;margin-top:10px">보낸 쪽지함</h2>
   <form method="get">
      <table style="margin-left:650px; margin-bottom:10px;">
         <tr>
            ...
            <td>
               <select name="user">
                  <option value="receiveId"<c:if test="${map.searchType=='receiveId'}">selected</c:if>>아이디</option>
      ...
      <table border="1" style="width:90%; margin-left:70px;" >
         <tr>
         ...
         <c:otherwise>
       	<c:forEach items="${sendList}" var="list" varStatus="stat">
	<tr>
		<td align="center"><input type="checkbox" name="checkbox" id="${list.msg_mnum},"></td>
		<td>${list.msg_receiveId}</td>
		<td><a href="<%=request.getContextPath()%>/pro/viewcon.do?mode=send&mnum=${list.msg_mnum}">${list.msg_title}</a></td>
...
function sendMnum(){
	...
	location.href='../pro/delete.do?mode=send&mnum='+checked;	
}
